...
import { updateAdsPaymentPlan } from "../actions/update.action";
...
export function EditAdzPaymentPlanDialog({ adsPaymentPlan }: Props) {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState({
    planName: adsPaymentPlan.planName,
    description: adsPaymentPlan.description || "",
    price: adsPaymentPlan.price,
    currency: adsPaymentPlan.currency,
    durationDays: adsPaymentPlan.durationDays,
    maxAds: adsPaymentPlan.maxAds,
    features: adsPaymentPlan.features || "",
    status: adsPaymentPlan.status,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateAdsPaymentPlan(adsPaymentPlan.id, {
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Make changes to the payment plan below.
            </DialogDescription>
          </DialogHeader>
