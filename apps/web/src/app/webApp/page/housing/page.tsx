  ...
  const scrollLeftPopular = () => {
    if (popularHomesScrollRef.current) {
      popularHomesScrollRef.current.scrollBy({
        left: -320, // Adjust this value based on card width + gap (min-w-px + space-x-6)
        behavior: "smooth",
      });
    }
  };
        ...
        */}
        ...
        <section className="mb-12">
          ...
          {loading ? (
            ...
            >
              {/* Ensure each HousingCard has a defined width within the flex container */}
              {housingItems.slice(0, 10).map(
                (
                  item // Example: showing first 10 items for "Popular"
                ) => (
                  <div
                    key={item.id}
