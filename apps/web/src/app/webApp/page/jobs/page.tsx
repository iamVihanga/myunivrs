...
//       {/* MODIFIED: Changed pb-[56.25%] to pb-[42.85%] for an even shorter 21:9 aspect ratio */}
...
//       // Adjusted scroll amount to match new card width + gap
...
//             >
//               {/* Width classes for JobCard wrapper remain w-[600px] */}
//               {jobItems.map((item) => (
//                 <div
//                   key={item.id}
//                   className="w-[500px] flex-shrink-0" // Set a fixed width to roughly double
//                 >
//                   <JobCard item={item} />
//                 </div>
//               ))}
...
export default function JobsPage() {
  ...
  const scrollLeft = () => {
    if (featuredJobsScrollRef.current) {
      const cardWidth = 500; // Card width as defined in className
      const gap = 24; // space-x-6 is 1.5rem = 24px
      featuredJobsScrollRef.current.scrollBy({
        left: -(cardWidth + gap),
        behavior: "smooth",
  ...
  // Function to scroll right, with looping
  const scrollRight = () => {
    if (featuredJobsScrollRef.current) {
      const container = featuredJobsScrollRef.current;
      const cardWidth = 500; // Card width as defined in className
      const gap = 24; // space-x-6 is 1.5rem = 24px
    ...
    const fetchJobData = async () => {
      ...
      try {
        ...
        const processedData = res.data
          ...
          .map((item) => ({
            ...item,
            postedDate: item.postedDate || "less than a minute ago",
            isFavorite: false,
  ...
  // Auto-scrolling effect for featured jobs
  useEffect(() => {
  ...
  }, [loading, jobItems.length]); // Re-run effect if loading state or item count changes
