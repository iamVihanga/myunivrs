...
import { db } from "@/db";
import { customer } from "@repo/database/schemas";
...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  const query = db.query.customer.findMany({
    limit: limitNum,
    offset,
  ...
  const [customers, _totalCount] = await Promise.all([query, totalCountQuery]);
...
export const create: AppRouteHandler<CreateRoute> = async (c) => {
  const customerEntry = c.req.valid("json");
